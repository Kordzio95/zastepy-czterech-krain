...
function drawUnit(u){
  ...
  const ang=u.facing;
  if(L.aura){
    const gl=.35+.25*Math.sin(TIME*3+u.id);
    cx.fillStyle=hexA(c.accent,gl*.5);
  ...
  if((dmgd||u.type==='heavy')&&!u.dead){
    const w=u.type==='heavy'?60:26, yy=sy-r-(u.type==='heavy'?20:12);
    cx.fillStyle='rgba(0,0,0,.6)'; cx.fillRect(sx-w/2,yy,w,4);
    cx.fillStyle=u.side==='player'?'#7ec96a':'#df5b4d';
    cx.fillRect(sx-w/2,yy,w*clamp(u.hp/u.maxHp,0,1),4);
  }
...
function drawSoldierTop(u,c,L,r,ang,hit){
  ...
  const body=hit?'#ffffff':(u.type==='archer'?shade(c.cloth,.06):c.cloth);
  // płaszcz
  if(L.cape){
    cx.fillStyle=hexA(c.main,.92);
    cx.beginPath(); cx.ellipse(-dx*r*.35,-dy*r*.35,r*.95,r*.8,ang,0,7); cx.fill();
  }
  // naramienniki
  if(L.pauldrons){
    cx.fillStyle=hit?'#fff':c.metal;
    for(const s of [-1,1]){
      const px=-dy*r*.72*s, py=dx*r*.72*s;
      cx.beginPath(); cx.ellipse(px,py,r*.4,r*.3,ang,0,7); cx.fill();
    }
  }
  // tors
  cx.fillStyle=hit?'#fff':(L.plate?c.metal:body);
  cx.beginPath(); cx.ellipse(0,0,r*.66,r*.56,ang,0,7); cx.fill();
  cx.strokeStyle='rgba(0,0,0,.35)'; cx.lineWidth=1.4; cx.stroke();
  if(L.plate){
    cx.fillStyle=hexA(c.main,.8);
    cx.beginPath(); cx.ellipse(dx*r*.12,dy*r*.12,r*.3,r*.24,ang,0,7); cx.fill();
  }
  // głowa / hełm
  const hx=dx*r*.16, hy=dy*r*.16;
  cx.fillStyle=hit?'#fff':(L.helmet?c.metal:c.skin);
  cx.beginPath(); cx.arc(hx,hy,r*.34,0,7); cx.fill();
  cx.strokeStyle='rgba(0,0,0,.4)'; cx.lineWidth=1.2; cx.stroke();
  if(L.helmet){
    cx.fillStyle=shade(c.metal,-.25);
    cx.beginPath(); cx.ellipse(hx+dx*r*.2,hy+dy*r*.2,r*.16,r*.24,ang,0,7); cx.fill();
  }
  if(L.plume){
    cx.strokeStyle=c.accent; cx.lineWidth=3;
    cx.beginPath(); cx.moveTo(hx,hy); cx.lineTo(hx-dx*r*.7,hy-dy*r*.7); cx.stroke();
  }
  if(u.faction==='nieumarli'&&!L.helmet){
    cx.fillStyle='#2b2440';
    cx.beginPath(); cx.arc(hx+dx*r*.16-dy*r*.1,hy+dy*r*.16+dx*r*.1,r*.06,0,7); cx.fill();
    cx.beginPath(); cx.arc(hx+dx*r*.16+dy*r*.1,hy+dy*r*.16-dx*r*.1,r*.06,0,7); cx.fill();
  }
  // broń
  const swing=u.atk>0?Math.sin((1-u.atk/u.ias)*Math.PI)*.9:0;
  if(u.type==='archer'){
    const bx=dx*r*.5, by=dy*r*.5;
    const pull=u.atk>0?clamp(u.atk/u.ias,0,1)*6:0;
    cx.strokeStyle=L.bigWeapon?shade(c.metal,-.1):'#7a5c34'; cx.lineWidth=L.bigWeapon?3.2:2.4;
    cx.beginPath(); cx.arc(bx,by,r*.62,ang-1.1,ang+1.1); cx.stroke();
    cx.strokeStyle=L.weaponGlow?hexA(c.accent,.95):'rgba(240,235,220,.85)'; cx.lineWidth=1.2;
    const ax=bx+Math.cos(ang-1.1)*r*.62, ay=by+Math.sin(ang-1.1)*r*.62;
    const bx2=bx+Math.cos(ang+1.1)*r*.62, by2=by+Math.sin(ang+1.1)*r*.62;
    cx.beginPath(); cx.moveTo(ax,ay); cx.lineTo(bx-dx*pull,by-dy*pull); cx.lineTo(bx2,by2); cx.stroke();
  } else if(u.type==='worker'){
    const a2=ang+swing*.7;
    cx.strokeStyle='#7a5c34'; cx.lineWidth=2.6;
    cx.beginPath(); cx.moveTo(Math.cos(a2)*r*.4,Math.sin(a2)*r*.4);
    cx.lineTo(Math.cos(a2)*r*1.15,Math.sin(a2)*r*1.15); cx.stroke();
    cx.fillStyle=c.metal;
    cx.beginPath(); cx.ellipse(Math.cos(a2)*r*1.2,Math.sin(a2)*r*1.2,r*.22,r*.13,a2,0,7); cx.fill();
  } else {
    const a2=ang+swing*1.2-.25;
    const len=r*(L.bigWeapon?1.5:1.2);
    if(L.weaponGlow){
      cx.strokeStyle=hexA(c.accent,.5); cx.lineWidth=7;
      cx.beginPath(); cx.moveTo(Math.cos(a2)*r*.4,Math.sin(a2)*r*.4); cx.lineTo(Math.cos(a2)*len,Math.sin(a2)*len); cx.stroke();
    }
    cx.strokeStyle=hit?'#fff':c.metal; cx.lineWidth=L.bigWeapon?4.4:3.2;
    cx.beginPath(); cx.moveTo(Math.cos(a2)*r*.4,Math.sin(a2)*r*.4); cx.lineTo(Math.cos(a2)*len,Math.sin(a2)*len); cx.stroke();
    // tarcza
    const s2=ang+1.5;
    cx.fillStyle=u.faction==='ludzie'?c.main:shade(c.metal,-.2);
    cx.beginPath(); cx.ellipse(Math.cos(s2)*r*.6,Math.sin(s2)*r*.6,r*.34,r*.24,s2,0,7); cx.fill();
    cx.strokeStyle='rgba(0,0,0,.4)'; cx.lineWidth=1.2; cx.stroke();
  }
  if(L.banner){
    const bxx=-dx*r*.8, byy=-dy*r*.8;
    cx.strokeStyle='#5a4a33'; cx.lineWidth=2;
    cx.beginPath(); cx.moveTo(bxx,byy); cx.lineTo(bxx-dy*10,byy+dx*10); cx.stroke();
    cx.fillStyle=hexA(c.accent,.9);
    cx.beginPath(); cx.moveTo(bxx-dy*10,byy+dx*10); cx.lineTo(bxx-dy*10-dx*9,byy+dx*10-dy*9);
    cx.lineTo(bxx-dy*4,byy+dx*4); cx.closePath(); cx.fill();
  }
}
...
function drawHeavyTop(u,c,L,r,ang,hit){
  ...
  if(L.aura){
    cx.fillStyle=hexA(c.accent,.18);
    cx.beginPath(); cx.arc(0,0,r*1.7,0,7); cx.fill();
  }
  // barki
  ...
  cx.strokeStyle='rgba(0,0,0,.4)'; cx.lineWidth=2; cx.stroke();
  if(L.armor){
  ...
  cx.beginPath(); cx.arc(hx,hy,r*.42,0,7); cx.fill();
